...
本项目为 OpenHarmony 平台编译了 ninja，并发布预构建包。

## 获取预构建包

前往 [release 页面](https://github.com/Harmonybrew/ohos-ninja/releases) 获取。
...
示例：
```sh
hdc file send ninja-1.13.1-ohos-arm64.tar.gz /data
hdc shell

cd /data
tar -zxf ninja-1.13.1-ohos-arm64.tar.gz
export PATH=$PATH:/data/ninja-1.13.1-ohos-arm64/bin
...
export PATH=$PATH:/opt/ninja-1.13.1-ohos-arm64/bin
...
需要用一台 Linux x64 服务器来运行项目里的 build.sh，以实现 ninja 的交叉编译。

这里以 Ubuntu 24.04 x64 作为示例：
```sh
sudo apt update && sudo apt install -y build-essential unzip cmake re2c
./build.sh
```
